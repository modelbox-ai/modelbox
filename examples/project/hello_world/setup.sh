...
main() {
    ...
    mv ${TARGE_DIR}/src/graph/hello_world.toml.in ${TARGE_DIR}/src/graph/hello_world.toml
    sed -i "s#@DEMO_HELLO_WORLD_FLOWUNIT_DIR@#@APPLICATION_PATH@/flowunit#g" ${TARGE_DIR}/src/graph/hello_world.toml
    if [ $? -ne 0 ]; then
