...
int _app_run_killcmd(struct app_monitor *app) {
  manager_log(MANAGER_LOG_INFO, "run kill command %s",
              strcmds(app->killcmd, sizeof(app->killcmd)));
  pid_t pid = vfork();
  if (pid < 0) {
    manager_log(MANAGER_LOG_ERR, "app %s killcmd failed, %s", app->name,
                strerror(errno));
    return -1;
  } else if (pid == 0) {
    /* close all fd */
...
int _app_stop(struct app_monitor *app, int gracefull) {
  ...
  if (killpg(app->pid, kill_mode) != 0) {
    ...
    manager_log(MANAGER_LOG_WARN, "app %s kill(%d) failed, pid %d, %s", app->name,
                kill_mode, app->pid, strerror(errno));
    return 0;
  }
