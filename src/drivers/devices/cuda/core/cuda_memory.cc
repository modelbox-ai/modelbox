...
Status CudaMemoryManager::Init() {
  ...
  std::call_once(flag, []() {
    GlobalRefVar<CudaMemoryPool, MAX_CUDA_DEVICE_NUMBER>::MakeFunc(
        [](int gpuid) -> std::shared_ptr<CudaMemoryPool> {
          std::string memorypool_name = "cuda_" + std::to_string(gpuid);
          auto pool = std::make_shared<CudaMemoryPool>(std::to_string(gpuid));
          if (pool->Init() != STATUS_OK) {
            return nullptr;
          }

          pool->RegisterCollector(memorypool_name);
          return pool;
        });
  });

  mem_pool_ = GlobalRefVar<CudaMemoryPool>::Get(gpu_id_);
  return STATUS_OK;
}
