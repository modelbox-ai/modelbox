...
modelbox::Status CarConditionFlowUnit::Process(
  ...
  for (size_t i = 0; i < bbox_input->Size(); ++i) {
    ...
    if (flag > 0) {
      ...
      auto shape_ret = buffer->Build(img_data_size + bbox_data_size);
      if (!shape_ret) {
        MBLOG_ERROR << "has_bbox_condition : get output memory failed.";
      ...
      auto bbox_ret = memcpy_s(buffer->MutableData(), bbox_data_size, bbox_data,
                               bbox_data_size);

      auto img_ret = memcpy_s((char *)buffer->MutableData() + bbox_data_size,
                              img_data_size, img_data, img_data_size);

      if (EOK != bbox_ret || EOK != img_ret) {
        MBLOG_ERROR << "Cpu has_bbox_condition failed, bbox_ret " << bbox_ret
                    << "img_ret" << img_ret;
