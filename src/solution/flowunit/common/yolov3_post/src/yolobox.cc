...
modelbox::Status YoloboxFlowUnit::Open(
  ...
  width_ = opts->GetUint32("input_width", 608);
  height_ = opts->GetUint32("input_height", 608);
  ...
  for (auto &layer : out_shape_) {
    int buf_size = layer.w * layer.h * layer.c * layer.c;
...
bool YoloboxFlowUnit::CheckShape(ImageShape shape, std::vector<size_t> input) {
...
modelbox::Status YoloboxFlowUnit::GetInferLayers(
    std::shared_ptr<modelbox::DataContext> data_ctx,
...
vector<BBox> YoloboxFlowUnit::ComputerBox(
    std::vector<std::shared_ptr<modelbox::BufferList>> &infer_layers,
    int batch_idx) {
  // one batch
  ...
  for (size_t i = 0; i < out_shape_.size(); ++i) {
    ...
    if (infer_outputs == nullptr) {
      MBLOG_WARN << "inifer output at " << i << ", " << batch_idx
                 << " is nullptr";
      continue;
    }
...
modelbox::Status YoloboxFlowUnit::SendBox(
    std::shared_ptr<modelbox::DataContext> data_ctx,
  ...
  for (size_t i = 0; i < shape.size(); ++i) {
    ...
    // out_data   output_bufs
    auto out_data = (BBox *)(*output_bufs)[i]->MutableData();
    if (out_data == nullptr) {
      MBLOG_ERROR << "output buffer is null";
      continue;
    }
...
MODELBOX_FLOWUNIT(YoloboxFlowUnit, desc) {
  desc.SetFlowUnitName(FLOWUNIT_NAME);
  desc.AddFlowUnitInput(
      modelbox::FlowUnitInput("layer82-conv", modelbox::DEVICE_TYPE));
  desc.AddFlowUnitInput(
      modelbox::FlowUnitInput("layer94-conv", modelbox::DEVICE_TYPE));
  desc.AddFlowUnitInput(
      modelbox::FlowUnitInput("layer106-conv", modelbox::DEVICE_TYPE));
