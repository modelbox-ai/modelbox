...
static void RefineBBox(BBox &bbox, int image_width, int image_height);

LPYoloboxFlowUnit::LPYoloboxFlowUnit(){};

LPYoloboxFlowUnit::~LPYoloboxFlowUnit(){};

modelbox::Status LPYoloboxFlowUnit::Open(
    const std::shared_ptr<modelbox::Configuration> &opts) {
  MBLOG_DEBUG << "lp_yolobox open ";
  ...
  width_ = opts->GetUint32("input_width", 416);
  height_ = opts->GetUint32("input_height", 416);
...
modelbox::Status LPYoloboxFlowUnit::Process(
    std::shared_ptr<modelbox::DataContext> data_ctx) {
  MBLOG_DEBUG << "process image lp_yolobox";

  // Input :  {[box], [box], [box]}
  auto input1_bufs = data_ctx->Input("car_bboxes");
  MBLOG_DEBUG << "lp_yolobox batch is: " << input1_bufs->Size();

  std::vector<std::shared_ptr<BBox>> batch_car_bboxes;
  for (size_t i = 0; i < input1_bufs->Size(); ++i) {
    if (input1_bufs->ConstBufferData(i) == nullptr) {
      continue;
    }
    // get car_boxes
    ...
    if (EOK != ret) {
      MBLOG_ERROR << "Cpu memcpy failed, ret " << ret << ", src size "
                  << sizeof(BBox) << ", dest size " << sizeof(BBox)
                  << ", skip an bbox!";
      continue;
    }

    batch_car_bboxes.push_back(b);
  }
  ...
  for (size_t batch_idx = 0; batch_idx < infer_layers[0]->Size(); ++batch_idx) {
    auto one_batch_bboxes =
        ComputerBox(infer_layers, batch_car_bboxes[batch_idx], batch_idx);
...
bool LPYoloboxFlowUnit::CheckShape(ImageShape shape,
...
modelbox::Status LPYoloboxFlowUnit::GetInferLayers(
    std::shared_ptr<modelbox::DataContext> data_ctx,
...
vector<BBox> LPYoloboxFlowUnit::ComputerBox(
    std::vector<std::shared_ptr<modelbox::BufferList>> &infer_layers,
  ...
  for (size_t i = 0; i < out_shape_.size(); ++i) {
    ...
    auto ret =
        memcpy_s(tmp.data(), tmp_buffer_size, infer_outputs, tmp_buffer_size);
    if (EOK != ret) {
      MBLOG_ERROR << "Cpu memcpy failed, ret " << ret << ", src size "
                  << tmp_buffer_size << ", dest size " << tmp_buffer_size
                  << ", skip infer output of layer[" << i << "] batch_idx["
                  << batch_idx << "]";
      continue;
    }
    // car boxes output size
    int bbox_w = car_bbox->w;
    int bbox_h = car_bbox->h;
    int bbox_x = car_bbox->x;
    int bbox_y = car_bbox->y;
    computeYoloBBox(param_, tmp.data(), i, layer_bboxes, bbox_w, bbox_h, bbox_x,
                    bbox_y, width_, height_, out_shape_[i].w, out_shape_[i].h,
                    out_shape_[i].c);
  ...
  for (auto box : one_batch_bboxes) {
    RefineBBox(box, image_width_, image_height_);
...
modelbox::Status LPYoloboxFlowUnit::SendBox(
    std::shared_ptr<modelbox::DataContext> data_ctx,
  ...
  for (int i = 0; i < model_param.anchor_num; ++i) {
    ...
    for (int h = 0; h < output_h; ++h)
      for (int w = 0; w < output_w; ++w) {
        ...
        bbox.x = std::max((bbox.x - shift_w - bbox.w / 2) * img_width + input_w,
                          0.0f);
        bbox.y = std::max(
            (bbox.y - shift_h - bbox.h / 2) * img_height + input_h, 0.0f);
        bbox.w *= (img_width);
        bbox.h *= (img_height);
...
static void RefineBBox(BBox &bbox, int image_width, int image_height) {
  float w = float(bbox.w * 0.1);
  float h = float(bbox.h * 0.15);
  bbox.x -= w;
  bbox.y -= h;
  bbox.h += 2 * h;
  bbox.w += 2 * w;
  bbox.x = std::max(bbox.x, float(0));
  bbox.y = std::max(bbox.y, float(0));
  bbox.w = std::min(bbox.w, image_width - bbox.x);
  bbox.h = std::min(bbox.h, image_height - bbox.y);
}

MODELBOX_FLOWUNIT(LPYoloboxFlowUnit, desc) {
  desc.SetFlowUnitName(FLOWUNIT_NAME);
  desc.AddFlowUnitInput(
      modelbox::FlowUnitInput("layer11-conv", modelbox::DEVICE_TYPE));
  desc.AddFlowUnitInput(
      modelbox::FlowUnitInput("car_bboxes", modelbox::DEVICE_TYPE));
